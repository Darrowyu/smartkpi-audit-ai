...
import { CreateAssignmentDto, UpdateAssignmentDto, BulkAssignmentDto, QueryAssignmentDto } from '../dto/assignment.dto';
...
export class AssignmentService {
    ...
    async create(dto: CreateAssignmentDto, companyId: string) {
        ...
        if (dto.departmentId && dto.employeeId) { // 检查分配层级互斥
            throw new BadRequestException('部门分配和个人分配不能同时指定');
        }

        const existing = await this.prisma.kPIAssignment.findFirst({ // 检查是否重复分配
            where: {
        ...
        const kpi = await this.prisma.kPIDefinition.findUnique({ where: { id: dto.kpiDefinitionId } }); // 获取默认权重
        ...
        return this.prisma.kPIAssignment.create({
            data: {
                ...
                weight: dto.weight ?? kpi?.defaultWeight ?? 10,
            },
    ...
    /** 复制上一周期的分配到新周期 */
    async copyFromPeriod(sourcePeriodId: string, targetPeriodId: string, companyId: string) {
    ...
    async getDepartmentAssignments(periodId: string, companyId: string) {
        ...
        return departments.map(dept => ({
            ...
            assignments: assignments.filter(a => a.departmentId === dept.id || !a.departmentId), // 部门级 + 公司级
    ...
    private async validatePeriodNotLocked(periodId: string, companyId: string) {
        ...
        if (period.status === PeriodStatus.LOCKED || period.status === PeriodStatus.ARCHIVED) {
