...
export class ExcelTemplateService {
  ...
  async generateTemplate(periodId: string, companyId: string): Promise<Buffer> {
    const [period, assignments, employees] = await Promise.all([
      // 获取周期和指标分配
    ...
    const headers = [
      ...
      '部门',
      ...kpiColumns.map(
        (k) => `${k.name}(目标: ${this.getTargetValue(assignments, k.id)})`,
      ),
    ];

    const data = employees.map((emp) => {
      // 构建数据行
      const row: any[] = [emp.employeeId, emp.name, emp.department?.name || ''];
