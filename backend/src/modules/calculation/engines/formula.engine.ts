...
export class FormulaEngine {
    ...
    calculatePositive(input: CalculationInput, cap = 120, floor = 0): CalculationResult {
        const rawScore = input.target === 0 ? 0 : (input.actual / input.target) * 100;
    ...
    calculateNegative(input: CalculationInput, cap = 120, floor = 0): CalculationResult {
        const rawScore = input.actual === 0 ? cap : (input.target / input.actual) * 100;
        const cappedScore = Math.max(floor, Math.min(rawScore, cap));
        const weightedScore = cappedScore * (input.weight / 100);
    ...
    /** 二元指标：发生即0分，未发生即满分 */
    calculateBinary(actual: number, weight: number, fullScore = 100): CalculationResult {
    ...
    /** 阶梯指标：根据阈值分段计分 */
    calculateStepped(actual: number, steps: StepRule[], weight: number): CalculationResult {
        let rawScore = 0;
    ...
    /** 自定义公式计算 */
    calculateCustom(formula: string, scope: Record<string, number>, weight: number, cap = 120, floor = 0): CalculationResult {
        try {
    ...
    /** 根据总分判定绩效等级 */
    determineStatus(totalScore: number): 'EXCELLENT' | 'GOOD' | 'AVERAGE' | 'POOR' {
    ...
    private evaluateStep(actual: number, step: StepRule): boolean {
        switch (step.operator) {
            case 'gte': return actual >= step.threshold;
            case 'gt': return actual > step.threshold;
            case 'lte': return actual <= step.threshold;
            case 'lt': return actual < step.threshold;
            case 'eq': return actual === step.threshold;
