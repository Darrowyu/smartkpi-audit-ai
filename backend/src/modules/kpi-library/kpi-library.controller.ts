...
export class KPILibraryController {
    ...
    async findAll(@Query() query: QueryKPIDefinitionDto, @Request() req: any) {
        return this.kpiLibraryService.findAll(query, req.user.companyId, req.user.groupId);
    }
    ...
    /** 更新指标 */
    @Put(':id')
    async update(@Param('id') id: string, @Body() dto: UpdateKPIDefinitionDto, @Request() req: any) {
    ...
    /** 批量导入指标 */
    @Post('bulk')
    async bulkCreate(@Body() definitions: CreateKPIDefinitionDto[], @Request() req: any) {
