...
import { CreateKPIDefinitionDto, UpdateKPIDefinitionDto, QueryKPIDefinitionDto } from './dto';
...
export class KPILibraryService {
    ...
    async create(dto: CreateKPIDefinitionDto, companyId: string) {
        const existing = await this.prisma.kPIDefinition.findFirst({ // 检查编码是否重复
        ...
        if (dto.customFormula) { // 验证自定义公式语法
    ...
    /** 获取指标列表（分页、搜索、筛选） */
    async findAll(query: QueryKPIDefinitionDto, companyId: string, groupId?: string) {
        const { search, formulaType, frequency, isActive, page = 1, limit = 20 } = query;
        const skip = (page - 1) * limit;

        const where = {
            AND: [
                { OR: [{ companyId }, { isGlobal: true }, ...(groupId ? [{ groupId }] : [])] }, // 权限过滤
