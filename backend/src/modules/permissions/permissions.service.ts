...
export class PermissionsService {
    ...
    async getRolePermissions(companyId: string): Promise<RolePermissions> {
        const config = await (this.prisma as any).permissionConfig.findUnique({ where: { companyId } });

        if (!config) return { ...DEFAULT_ROLE_PERMISSIONS }; // 返回默认配置

        const stored = config.rolePermissions as RolePermissions;
        return { // 合并默认配置和自定义配置（SUPER_ADMIN 始终保持完整权限）
            ...stored,
            [UserRole.SUPER_ADMIN]: ALL_PERMISSIONS.map(p => p.id),
        };
    }
    ...
    /** 保存公司的角色权限配置 */
    async saveRolePermissions(companyId: string, rolePermissions: RolePermissions, userId: string): Promise<void> {
        const data: any = { // 过滤掉 SUPER_ADMIN（不允许修改）
            ...rolePermissions,
    ...
    /** 检查用户是否有特定权限 */
    async checkPermission(companyId: string, userRole: UserRole, permissionId: string): Promise<boolean> {
    ...
    /** 获取用户的所有权限 */
    async getUserPermissions(companyId: string, userRole: UserRole): Promise<string[]> {
        if (userRole === UserRole.SUPER_ADMIN) return ALL_PERMISSIONS.map(p => p.id);
    ...
    async resetToDefault(companyId: string): Promise<void> {
        await (this.prisma as any).permissionConfig.deleteMany({ where: { companyId } });
    }
}
