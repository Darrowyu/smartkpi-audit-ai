...
export class ExportService {
    ...
    async exportEmployeePerformance(options: ExportOptions): Promise<Buffer> {
        ...
        const data = performances.map((p, index) => { // 构建导出数据
    ...
    private generateExcel(data: Record<string, any>[], format: 'xlsx' | 'csv', sheetName: string): Buffer {
        const ws = XLSX.utils.json_to_sheet(data);

        const colWidths = Object.keys(data[0] || {}).map(key => ({ wch: Math.max(key.length * 2, 10) })); // 自动列宽
        ws['!cols'] = colWidths;
