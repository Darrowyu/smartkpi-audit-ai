...
export class ReportsController {
    ...
    /** 获取绩效趋势 */
    @Get('trend')
    async getPerformanceTrend(@Query('periodCount') periodCount: string, @Request() req: any) {
        return this.reportsService.getPerformanceTrend(req.user.companyId, +periodCount || 6);
    }
    ...
    async getLowPerformanceAlerts(
        ...
        return this.reportsService.getLowPerformanceAlerts(periodId, req.user.companyId, +threshold || 60);
    }

    /** 获取集团级汇总（GROUP_ADMIN 权限） */
    @Get('group-overview')
    async getGroupOverview(@Query('periodName') periodName: string, @Request() req: any) {
