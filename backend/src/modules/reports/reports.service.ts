...
export class ReportsService {
  ...
  async getPeriodOverview(
    ...
    if (userContext) {
      ...
      if (userContext.role === UserRole.USER && userContext.linkedEmployeeId) {
        whereClause = {
          ...whereClause,
          employeeId: userContext.linkedEmployeeId,
        };
      } else if (
        userContext.role === UserRole.MANAGER &&
    ...
    const [period, performances] = await Promise.all([
      this.prisma.assessmentPeriod.findFirst({
    ...
    return {
      ...
      excellent: performances.filter(
        (p) => p.status === KPIStatusEnum.EXCELLENT,
      ).length,
      good: performances.filter((p) => p.status === KPIStatusEnum.GOOD).length,
      average: performances.filter((p) => p.status === KPIStatusEnum.AVERAGE)
        .length,
  ...
  async getEmployeeRanking(
    ...
    limit = 20,
    userContext?: UserContext,
  ): Promise<{ data: EmployeeRanking[]; total: number; isFiltered?: boolean }> {
  ...
  async getPerformanceTrend(
    ...
    for (const period of periods.reverse()) {
      ...
      if (performances.length > 0) {
        const avgScore =
          performances.reduce((sum, p) => sum + p.totalScore, 0) /
          performances.length;
        trends.push({
          period: period.name,
          avgScore: Math.round(avgScore * 100) / 100,
  ...
  async getGroupOverview(groupId: string, periodName?: string) {
    ...
    const results: (PerformanceOverview & {
      companyId: string;
      companyName: string;
    })[] = [];
    ...
    for (const company of companies) {
      const period = await this.prisma.assessmentPeriod.findFirst({
        where: {
          ...
          ...(periodName && { name: { contains: periodName } }),
        },
        orderBy: { endDate: 'desc' },
      });
